module Pod
  class Builder
    ...
    def compile(platform)
      ...
      if ENV['DONOTUSEMODERNBUILDSYSTEM']
        if options
          options << ' -UseModernBuildSystem=NO'
        end
      end
...
MAP
    ...
    def ios_build_options
      if ENV['DONOTUSEBITCODE']
      	"ARCHS=\'x86_64 i386 arm64 armv7 armv7s\' OTHER_CFLAGS=\'-Qunused-arguments\'"
      else
    ...
    def xcodebuild(defines = '', args = '', build_dir = 'build', target = 'Pods-packager', project_root = @static_sandbox_root, config = @config)
      ...
      if ENV['DONOTUSEMODERNBUILDSYSTEM']
        command = "xcodebuild #{defines} #{args} -UseModernBuildSystem=NO CONFIGURATION_BUILD_DIR=#{build_dir} clean build -configuration #{config} -target #{target} -project #{project_root}/Pods.xcodeproj 2>&1"
